...
var getHistories = function (modelName, id, expandableFields, limit, offset, callback) {
    History.find({collectionName: modelName, collectionId: id}).limit(limit).skip(offset).exec(function (err, histories) {
        if (err) {
            console.error(err);
            return callback(err, null);
        }
        async.map(histories, function (history, mapCallback) {
            ...
            for (var key in history.diff) {
                if (history.diff.hasOwnProperty(key)) {
                    if ((isEmpty(expandableFields) == false && expandableFields.indexOf(key) < 0) || key == "updatedAt" || key == "_id") {

                    }
                    else {
                      //console.log(history.diff[key]);
                      if (history.diff[key] !== null && typeof history.diff[key] === 'object' && !(history.diff[key] instanceof Array)) {

                        changes[key] = {};
                        for (var skey in history.diff[key]) {
                          if (history.diff[key].hasOwnProperty(skey) && skey != "_t") {
                            if (skey == "0") {
                              changes[key]["type"] = "update";
                              if (history.diff[key][skey] !== null && typeof history.diff[key][skey] === 'object' && !(history.diff[key][skey] instanceof Array)) {
                                for (var newkey in history.diff[key][skey]) {
                                  if (history.diff[key][skey].hasOwnProperty(newkey)) {
                                    changes[key].property = newkey;
                                    changes[key].old = history.diff[key][skey][newkey][0]
                                    changes[key].new = history.diff[key][skey][newkey][1]
                                  }
                                }
                              } else {
                                changes[key]["type"] = "create";
                                changes[key]["property"] = history.diff[key][skey][0]["Name"];
                              }
                            } else if (skey == "_0") {
                      ...
                      } else if (history.diff[0] == null && history.diff[1] != null) {
                        changes["key"] = {};
                        changes["key"].type = "create";
                        changes["key"].property = "Experiment";
                        changes["key"].new = history.diff[1].Name;
                      }
                      else {
                        changes[key] = {};
                        ...
                        if (key == "Thumbnail") {
                          changes[key].new = "New Thumnail"
                          changes[key].old = "New Thumnail"
                        }
                        else {
